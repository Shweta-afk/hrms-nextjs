    ...
    <footer className="border-t border-zinc-200 dark:border-zinc-800">
      <div className="mx-auto max-w-6xl px-6 py-16">
              ...
              <ul className="mt-4 space-y-2.5">
                {c.links.map((l) => (
                  <li key={l.label}>
                    <Link
                      href={l.href}
                      className="text-sm text-zinc-700 transition-colors hover:text-indigo-600 focus-visible:outline-none focus-visible:text-indigo-600 dark:text-zinc-300"
                    >
                      {l.label}
                    </Link>
                  </li>
        ...
        <div className="mt-14 border-t border-zinc-200 pt-6 text-sm text-zinc-500 dark:border-zinc-800">
          <p>© {new Date().getFullYear()} Acme HR. Made in Bengaluru 🇮🇳</p>
        </div>
      </div>
    </footer>
  );
