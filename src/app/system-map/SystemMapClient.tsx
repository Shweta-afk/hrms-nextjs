...
const NODES: NodeDef[] = [
  ...
  // ── HR Admin Pages ───────────────────────────────────────────────────
  {
    ...
    info: {
      ...
      fix: ['Re-login to refresh stale JWT', 'If org_id mismatch: check users.org_id FK integrity', 'Widget timeout: check Neon DB connection pool (max 10 on free plan)'],
  ...
  },
  {
    ...
    info: {
      ...
      diagnose: ['DevTools Network — check /api/analytics/* endpoints for errors or timeouts', 'Check Neon DB connection pool — analytics queries are heavy (use EXPLAIN on slow queries)', 'Check perf indexes migration (20260604000001_perf_indexes) was applied'],
  ...
  },
  {
    ...
    info: {
      ...
      diagnose: ['Check Neon DB logs for constraint violations during run', 'S3: check AWS credentials, bucket name, and region in env vars', 'Check RESEND_API_KEY for approval notifications'],
  ...
  // ── Infrastructure ────────────────────────────────────────────────────
  {
    id: 'neon-db', label: 'Neon PostgreSQL', sublabel: 'Prisma ORM · multi-tenant · RLS',
    x: 200, y: 680, layer: 'infra',
    info: {
      what: 'Neon serverless PostgreSQL. All app data. Prisma as ORM. Multi-tenancy via org_id on every table. RLS enabled. Free tier: 0.5 GB storage, 10 connections. DIRECT_URL for migrations.',
      breaks: ['All DB queries time out', '"Too many clients" error in logs', 'Migration fails to apply', 'Free tier storage limit hit'],
      diagnose: ['Neon dashboard → check connection pool and active connections', 'prisma db execute to test connectivity', 'Check prisma/migrations/ — compare applied migrations with schema', 'Neon dashboard → Storage tab'],
      fix: ['Connection pool exhausted: add connection_limit=5 to DATABASE_URL', 'Migration drift: npx prisma migrate deploy', 'Storage full: archive old punch_logs, rate_limit_entries, notifications', 'Upgrade Neon plan for more connections and storage'],
...
const EDGES: EdgeDef[] = [
  ...
  // infra internal
  { from: 'neon-db',         to: 'vercel',          dashed: true },
      ...
      <div style={{ padding: '8px 22px 0' }}>
        ...
        <div style={{ background: page.surface, border: `1px solid ${page.border}`,
          ...
          {[
            ...
            { key: 'DATABASE_URL',              where: 'Vercel env',   how: 'Neon dashboard → Connection string',                 affects: 'Entire app — no DB = no app' },
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10 }}>
          {[
            { title: 'User Login', color: colors.public.border, steps: [
              ...
              '→ peekRateLimit() — checks per-email + per-IP buckets in Neon',
